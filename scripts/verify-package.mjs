...
try {
  ...
  await writeFile(
    ...
    [
      "import { ContractRegistryError, SUPPORTED_SCHEMA_VERSION, assertContractGraph, assertV2Contract, assertV3CalibrationResult, validateAllSchemas, validateContract, validateContractGraph, validateV2Contract, validateV3CalibrationResult } from 'mercury-subtitles';",
      ...
      "if (![commonSchema, modelConfigSchema, modelSnapshotSchema, transcriptSchema, calibrationSchema, audioVerificationSchema, modelConfigV2Schema, modelSnapshotV2Schema, calibrationV2Schema, calibrationV3Schema].every((schema) => schema.$id)) throw new Error('schema export is unavailable');",
      'validateAllSchemas();',
  ...
  await writeFile(
    ...
    [
      "import type { CalibrationResultV2, ModelConfigRegistryV2, ModelSnapshotV2 } from 'mercury-subtitles';",
      ...
      'void [config, snapshot, calibration, subtitle, srt, plugin];',
    ].join('\n'),
  );
  await writeFile(
...
console.log(
  `Package ${packageJson.name}@${productVersion} contains runnable JavaScript, declarations, v1-v4 schemas, the Mercury subtitle Skill, and an isolated CLI consumer.`,
);
